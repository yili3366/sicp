...
;;  (put '=zero?     '(rational)           (lambda (x)   (=zero-rat x)))
  ...
  (put 'type-level '(rational)
       (lambda (x) 2))
;;  (put 'project    '(rational)           (lambda (x)   (project x)))
  (put 'sq-root '(rational)
  ...
  (put 'arctan '(rational rational)
       (lambda (x y) (make-real (atan (ratio x) (ratio y)))))
  (put 'negate '(rational)
       (lambda (x)   (sub (tag (make-rat 0 1)) x)))
  'done)
...
(define (install-dense-poly-package)
  ...
  (put 'reduce-poly '(dense dense)
       (lambda (p1 p2)
         (let ((new-ps (reduce-poly p1 p2)))
           (list (tag (car new-ps))
                 (tag (cadr new-ps))))))
  (put 'reduce-poly '(dense sparse)
       (lambda (p1 p2)
         (let ((new-ps (reduce-poly p1 (dense->sparse) p2)))
           (list (tag (car new-ps))
                 (tag (cadr new-ps))))))
...
(define (install-sparse-poly-package)
  ...
  (put 'reduce-poly '(sparse sparse)
       (lambda (p1 p2)
         (let ((new-ps (reduce-poly p1 p2)))
           (list (tag (car new-ps))
                 (tag (cadr new-ps))))))
  (put 'reduce-poly '(sparse dense)
       (lambda (p1 p2)
         (let ((new-ps (reduce-poly p1 (sparse->dense) p2)))
           (list (tag (car new-ps))
                 (tag (cadr new-ps))))))
